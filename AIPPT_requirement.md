...
@meta
version: 2.0.0
last-updated: 2026-04-26 18:00:00
...
| 搜索配图 | "帮我找几张教室的图片" | LLM 调用 search_images → 图片网格 |
...
用户输入消息
  ...
  └─ 第二层：LLM Function Calling
      ...
      ├─ LLM 返回 search_images → 调用图片 API → 显示图片网格
...
| 参数 | 值 |
|------|-----|
| API 端点 | 开发环境 `/llm/v1/chat/completions`，生产环境 `https://modelproxy.unipus.cn/v1/chat/completions` |
| 模型 | `qwen-plus` |
| 认证 | Bearer Token |
...
### 2.4 幻灯片上下文注入（describeCurrentSlide）
...
## 第 5 章：Function Calling 工具定义

### 5.1 工具列表

以下 8 个工具通过 OpenAI 兼容的 Function Calling 机制提供给 LLM。请求时放在 `tools` 参数中，`tool_choice: 'auto'`。

#### 5.1.1 update_element — 修改已有元素
...
#### 5.1.3 add_image_element — 添加图片

参数：`src`(必填), `left`(默认300), `top`(默认150), `width`(默认400), `height`(默认300), `radius`

#### 5.1.4 add_slide — 新建页面

参数：`background_color`(默认#ffffff), `remark`, `elements[]`（每个元素含 type/位置/内容/样式）

#### 5.1.5 delete_element

参数：`element_id`(必填)

#### 5.1.6 update_slide_background

参数：`color`(必填)

#### 5.1.7 update_slide_remark

参数：`remark`(必填)

#### 5.1.8 search_images — 搜索图片

```json
{
  "name": "search_images",
  "description": "搜索图片。当用户要求查找、搜索图片或需要配图时调用。",
  "parameters": {
    "properties": {
      "query": { "type": "string", "description": "搜索关键词（英文效果更好）" },
      "orientation": { "enum": ["landscape", "portrait", "square", "all"], "default": "landscape" }
    },
    "required": ["query"]
  }
}
...
search_images 的执行不在 executeTool 中，而是在 AIChatFloating.vue 中特殊处理：调用 Pexels API 获取 4 张图片，在聊天框中显示 2×2 网格，用户点击后插入。

### 5.2 工具执行后的二次 LLM 调用

当 LLM 返回 tool_calls 时（非 search_images），执行流程为：
...
- 每个会话包含：唯一 ID、标题（首条消息前 20 字）、绑定元素 ID（可选）、消息列表、创建时间
- 持久化：sessionStorage（关闭浏览器丢失）
- 历史消息截断：发送给 LLM 时只取最近 10 条
