...
- 每次打开对话框位置是固定的，防止用户拖动到屏幕外后再次打开还在屏幕外

### 2.5 语音输入

| 属性 | 说明 |
|------|------|
| 语言 | 中文（zh-CN），保留英文可能性，未来可能要支持英文（外教） |
| 入口 | 侧边栏输入框旁 + 悬浮框输入框旁 |
...
messages = [
  {
    role: "system",
    content: SYSTEM_PROMPT                          ← 完整 System Prompt（见第 4 章）
           + "\n\n## 当前PPT模板信息\n"
...
## 第 11 章：跨项目架构设计 — 统一智能体内核 [Updated 2026-04-27]
...
两者核心流程同构：**理解意图 → 生成内容 → 适配格式 → 写入宿主系统**，差异在于"生成什么内容"和"写入哪个系统"。经评估，两个智能体应共享底层能力，前端各自适配宿主平台。
...
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌────────────┐  │
│  │ U校园备课端      │  │ iPublish        │  │ 未来平台... │  │
│  │ PPTist编辑器    │  │ 图书编辑平台     │  │            │  │
│  └────────┬────────┘  └────────┬────────┘  └─────┬──────┘  │
│           │                    │                  │         │
│  暴露内容写入标准（Function Calling 接口）                      │
└───────────┼────────────────────┼──────────────────┼─────────┘
            │                    │                  │
┌───────────▼────────────────────▼──────────────────▼─────────┐
│  第三层：写入适配层（路由层）                                    │
│                                                             │
│  接收 AgentContext（平台、角色、任务类型）                       │
│  根据用户所在平台和身份组装信息                                  │
│  选择对应平台的 Function Calling 工具集                         │
│  将内容转换为符合该平台标准的格式后返回给业务                      │
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐                  │
│  │ PPTist Writer   │  │ Lexical Writer   │                  │
│  │ addSlide        │  │ insertParagraph  │                  │
│  │ insertText...   │  │ insertTable      │                  │
│  │ updateElement   │  │ replaceSelection │                  │
...
│  第二层：内容生产层（Skills 库）                                │
│                                                             │
│  拥有众多 Skills，根据用户意图调用对应 Skill 生成内容             │
│                                                             │
│  简单 Skill：配置提示词即可生成（课堂引入、总结要点等）            │
│  复杂 Skill：需调用外部接口获取内容（素材搜索、知识库检索等）       │
│                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ 备课技能集    │  │  共享技能     │  │教材编写技能集 │      │
│  │              │  │              │  │              │      │
...
│    · 简单格式指令 → 正则匹配，零延迟本地执行                     │
│    · 简单内容生成 → 小模型快速响应                               │
│    · 复杂内容生成 → 大模型 + Function Calling                   │
│    · 闲聊/问答 → 大模型直接回复                                 │
│                                                             │
│  [Session 管理] [意图识别] [模型路由] [教育知识库]               │
└─────────────────────────────────────────────────────────────┘
```
...
这是整个系统的入口和基座，负责接收、存储、路由所有平台的用户会话。

| 模块 | 职责 | 说明 |
|------|------|------|
| Session 管理 | 会话创建/切换/历史/持久化 | 统一多 session 模型，存储各平台来的会话 |
| 意图识别 | 判断用户意图类型 | 生成内容 / 修改格式 / 查询 / 闲聊 |
| 模型路由 | 根据意图选择调用路径 | 正则匹配（零延迟）/ 小模型（快速）/ 大模型（复杂） |
| 教育知识库 | 教学法与学科知识 | POA / PBL / 场景教学法，各平台共用 |

**模型路由决策**：

| 用户意图 | 调用路径 | 说明 |
|----------|---------|------|
| 简单格式指令（"字号改成30"） | 正则匹配，本地执行 | 零延迟，零 token |
| 简单内容生成 | 小模型快速响应 | 低延迟，低成本 |
| 复杂内容生成/元素操作 | 大模型 + Function Calling | 高质量，支持工具调用 |
| 闲聊/问答 | 大模型直接回复 | 不需要工具 |

**Session 模型统一**：以多 session 模型为基准。备课场景多 session（每个任务独立会话），教材编写场景默认单 session（一本书一个长会话，单 session 是多 session 的特例）。

#### 11.3.2 第二层：内容生产层（Skills 库）

这是整个系统的核心价值所在。拥有众多 Skills，根据用户意图调用对应 Skill 生成内容。

**Skill 的两种类型**：

| 类型 | 说明 | 示例 |
|------|------|------|
| 简单 Skill | 通过配置提示词模板即可生成内容 | 课堂引入生成、总结要点、演讲稿生成 |
| 复杂 Skill | 需要调用外部接口获取数据后再生成 | 素材搜索（调用素材库 API）、知识库检索（调用知识库 API）、出版规范审核（调用审核服务） |

**Skill 按业务分组**：

| 技能集 | 包含的 Skills | 适用平台 |
|--------|-------------|---------|
| 备课技能集 | 课堂引入生成、教学活动设计、幻灯片布局建议 | U 校园备课端 |
| 教材编写技能集 | 文章生成、素材推荐、体例检查、审校建议、出版规范审核、涉政涉密检测 | iPublish |
| 共享技能 | 习题生成、内容润色、总结要点、演讲稿生成、难度调整、翻译 | 所有平台 |

共享技能是合并的核心收益——一次开发，所有平台复用。

**审核类 Skill 的特殊说明**：
- 审核本质上和润色、难度调整一样，是一种内容处理 Skill
- 教材编写场景可配置为"生成 → 自动审核 → 输出"的 Skill 编排链
- 备课场景审核为可选项，教师可主动触发
- 审核严格程度由业务场景决定，不是架构层面的硬约束

#### 11.3.3 第三层：写入适配层（路由层）

根据用户所在的平台和身份等信息，组装上下文，选择对应平台的 Function Calling 工具集，将内容转换为符合该平台标准的格式后返回给业务层。

**核心职责**：
1. 接收 AgentContext（平台、角色、任务类型）
2. 选择对应平台的 Writer（Function Calling 工具集）
3. 将第二层生成的内容通过 Writer 转换为平台标准格式
4. 返回给业务层写入
...
```
AgentContext:
  platform: 'ucampus' | 'ipublish'   ← 用户所在平台
...
| Writer | 宿主平台 | 典型 Functions |
|--------|---------|---------------|
| PPTist Writer | U 校园备课端 | addSlide, insertTextElement, updateElement, applyTheme, updateSlideBackground |
| Lexical Writer | iPublish | insertParagraph, insertTable, insertImage, replaceSelection, applyStyle |

LLM 不需要知道底层是 Lexical 还是 PPTist，路由层根据场景只注册对应的那套 Functions。

#### 11.3.4 第四层（最顶层）：业务场景层

各业务平台作为最顶层，暴露自己的内容写入标准（Function Calling 接口），让大模型可以写入和编辑自己平台的内容。

| 平台 | 编辑器 | 暴露的写入标准 |
|------|--------|--------------|
| U 校园备课端 | PPTist | 幻灯片操作接口（增删改页面、元素、背景、备注） |
| iPublish | Lexical | 富文本操作接口（段落、表格、图片、样式） |
| 未来平台 | 待定 | 只需实现自己的 Writer 即可接入 |

新平台接入只需要：实现一套 Writer（Function Calling 接口）+ 注册到路由层，无需改动第一层和第二层。
...
2. 第一层(会话管理层): 存储会话 → 意图识别(内容生成) → 路由到大模型
3. 第二层(内容生产层): 调用备课技能集 → design_activity Skill
   → LLM 生成活动内容（融入 POA 驱动 + 场景教学法）
4. 第三层(写入适配层): AgentContext{platform:'ucampus'}
   → 选择 PPTist Writer → addSlide() + insertTextElement()
5. 第四层(业务场景层): U校园备课端接收写入 → 新幻灯片插入
   → 显示"插入当前页"/"新建页插入"按钮
```
...
2. 第一层(会话管理层): 存储会话 → 意图识别(内容生成) → 路由到大模型
3. 第二层(内容生产层): 调用教材编写技能集 → generate_article Skill
   → LLM 生成语法讲解内容
   → Skill 编排链自动触发: 出版规范审核 + 涉政涉密检测
   → [通过] 继续 / [不通过] 带修正指令重新生成或返回审核意见
4. 第三层(写入适配层): AgentContext{platform:'ipublish'}
   → 选择 Lexical Writer → insertParagraph() + applyStyle()
5. 第四层(业务场景层): iPublish 接收写入 → 内容插入编辑器光标位置
```

#### 案例 3：共享 Skill 复用

```
1. 用户(教师或编辑): "生成 3 道关于 Past Tense 的选择题"
2. 第一层(会话管理层): 存储会话 → 意图识别(内容生成) → 路由到大模型
3. 第二层(内容生产层): 调用共享技能 → generate_exercise Skill
   → LLM 生成习题（场景教学法 + POA 促成）
4. 第三层(写入适配层): 根据 platform 选择 Writer
   → ucampus: PPTist Writer → 每道题一张幻灯片
   → ipublish: Lexical Writer → 习题插入文档章节
5. 第四层(业务场景层): 对应平台接收写入
```
...
| 阶段 | 内容 | 优先级 |
|------|------|--------|
| 第一步 | 统一第二层 Skills 库：共享 prompt 模板、教育知识库 | 高 — 改动最小，收益最大 |
| 第二步 | 定义第三层统一的 Writer 注册协议，各平台以插件形式接入 | 中 — 需要定义接口规范 |
| 第三步 | 抽取共享 Skills（习题生成、内容润色等），统一维护 | 中 — 逐步迁移 |
| 第四步 | 统一第一层会话管理，多 session 模型覆盖所有场景 | 低 — 各平台需配合改造 |
...
| 2026-04-27 | incremental | 2.4.0 | 规范第11章四层架构定义：第一层(底层)会话管理层、第二层内容生产层(Skills库)、第三层写入适配层(路由层)、第四层(最顶层)业务场景层；补充模型路由决策（正则/小模型/大模型）、Skill两种类型（简单/复杂）、新平台接入方式 |
