...
import { X, Search, Play, Music, FileText, Image } from 'lucide-react';
...
interface Resource {
  id: string; title: string; type: MediaType; source: '外研社' | '三方'; thumb: string; desc: string;
}
...
const TICON: Record<string, React.ReactNode> = {
  '视频': <Play className="w-3.5 h-3.5" />, '音频': <Music className="w-3.5 h-3.5" />,
  '题目': <FileText className="w-3.5 h-3.5" />, '图片': <Image className="w-3.5 h-3.5" />,
};
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} transition={{ duration: 0.2 }}
            className="bg-white rounded-xl shadow-2xl w-[720px] h-[560px] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
