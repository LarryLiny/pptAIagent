...
export function AIChatPanel() {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      if (ct === '生成课堂引入') {
        const it = st.introType || '情境导入'; const df = st.difficulty || '适中';
        add({ type: 'ai', content: '好的，已为您生成「' + it + '」风格、「' + df + '」难度的课堂引入：\n\n「Good morning, class! Before we begin today\'s lesson on Environmental Protection, let me show you something. Last week, a massive wildfire broke out in Australia, destroying thousands of hectares of forest. What do you think caused this disaster? And more importantly \u2014 what can we do about it? Today, we\'ll explore these questions together and learn how to express our ideas about protecting our planet in English.」', image: 'https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=800', buttons: [{ label: '插入到PPT', action: 'insert' }, { label: '重新生成', action: 'regenerate' }] });
        return;
      }
      if (ct === '生成演讲稿') { add({ type: 'ai', content: '以下是为您生成的课堂演讲稿：\n\n「同学们好，今天我们来学习环境保护这一主题。大家知道，全球变暖正在影响我们的生活。接下来，我们将通过几个案例了解如何用英语表达环保理念，并探讨我们能为地球做些什么。」', buttons: [{ label: '插入到备注', action: 'insert-note' }, { label: '重新生成', action: 'regenerate-speech' }] }); return; }
      if (txt.includes('难度调低') || txt.includes('难度降低')) { add({ type: 'ai', content: '好的，已根据外研社学生能力标准为您调整文本难度：\n\n「We should protect the environment. Trees give us clean air. We can save water and use less plastic. Small actions can make a big difference. Let\'s work together to keep our Earth clean and green.」', buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-easy' }] }); return; }
      if (txt.includes('相似题')) { add({ type: 'ai', content: '好的，已从外研社题库中为您匹配到一道相似题：\n\nChoose the correct answer:\nIf I ___ more time yesterday, I would have finished the project.\nA. have  B. had had  C. had  D. would have\n\nAnswer: B', buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-quiz' }] }); return; }
      if (txt.includes('爱迪生') || txt.includes('Edison')) { add({ type: 'ai', content: '为您找到以下资料：\n\n「Thomas Edison (1847\u20131931) was an American inventor who developed the phonograph, the motion picture camera, and the practical electric light bulb. He held over 1,000 patents.」', image: edisonImg, buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-edison' }] }); return; }
      if (txt.includes('俄乌') || txt.includes('Russia')) { add({ type: 'ai', content: '好的，为您生成了一段引入短文：\n\n「Russia is the largest country in the world. Recently, the conflict between Russia and Ukraine has drawn global attention. This event reminds us of the importance of peace and diplomacy.」', image: conflictImg, buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-russia' }] }); return; }
      if (step === 0) { add({ type: 'ai', content: '好的，通过搜索外研社的素材库，推荐插入以下内容：\n\n「In recent years, the importance of environmental protection has become increasingly evident. Education plays a crucial role in raising awareness about environmental issues.」', buttons: [{ label: '插入', action: 'insert' }, { label: '重新生成', action: 'regenerate' }] }); setStep(1); }
  ...
  const onBtn = (a: string) => {
    const R: Record<string, () => void> = {
      'agree': () => setTimeout(() => add({ type: 'ai', content: '已为您创建新的一页PPT并插入内容。' }), 800),
      ...
      'regenerate-easy': () => add({ type: 'ai', content: '好的，重新生成了更简单的版本：\n\n「Our planet needs help. We can plant trees, save water, and recycle. Let\'s protect our beautiful Earth together.」', buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-easy' }] }),
      ...
      'regenerate-edison': () => add({ type: 'ai', content: '好的，重新生成了爱迪生资料：\n\n「Edison was a self-taught genius. His most famous invention, the light bulb, took thousands of experiments.」', buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-edison' }] }),
      'regenerate-russia': () => add({ type: 'ai', content: '好的，重新生成了引入短文：\n\n「The word Russia comes from "Rus," a medieval state. The ongoing Russia-Ukraine conflict has become a major topic in international news.」', buttons: [{ label: '替换文本', action: 'replace-text' }, { label: '重新生成', action: 'regenerate-russia' }] }),
      'regenerate-speech': () => add({ type: 'ai', content: '好的，我为您重新生成演讲稿：\n\n「各位同学，欢迎来到今天的英语课。本节课我们聚焦环境保护话题。准备好了吗？Let\'s go!」', buttons: [{ label: '插入到备注', action: 'insert-note' }, { label: '重新生成', action: 'regenerate-speech' }] }),
      'regenerate': () => add({ type: 'ai', content: '好的，我为您重新生成内容：\n\n「Climate change represents one of the most pressing challenges of our time.」', buttons: [{ label: '插入', action: 'insert' }, { label: '重新生成', action: 'regenerate' }] }),
    }; R[a]?.();
  };

  const ts = tool ? TOOL_SETTINGS[tool] : null;
  ...
  return (
    <div className="h-full flex flex-col bg-white">
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden border-t border-gray-200">
            <div className="px-4 py-3 bg-gray-50 max-h-48 overflow-y-auto space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-medium text-purple-700 flex items-center gap-1"><Sparkles className="w-3.5 h-3.5" />{tool} · 高级配置</div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {s.options.map(o => (
                      <button key={o} onClick={() => updateSetting(s.key, o)}
                        className={`px-2.5 py-1 rounded-full text-xs transition-all ${settings[s.key] === o ? 'bg-purple-500 text-white shadow-sm' : 'bg-white border border-gray-300 text-gray-600 hover:border-purple-300 hover:text-purple-600'}`}>
                        {o}
                      </button>
                    ))}
                  </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="relative">
              ...
              <AnimatePresence>{toolMenu && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} onMouseEnter={() => setToolMenu(true)} onMouseLeave={() => setToolMenu(false)} className="absolute bottom-full left-0 mb-1 w-40 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden z-10">
                  {TL.map(t => <button key={t} onClick={() => selectTool(t)} className="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-2"><span className="text-gray-400">{TI[t]}</span>{t}</button>)}
                </motion.div>
              )}</AnimatePresence>
            </div>
          </div>
          <div className="relative mr-2">
            <button onClick={() => setHelpOpen(!helpOpen)} className="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-300 rounded-lg text-xs text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-colors whitespace-nowrap"><HelpCircle className="w-3.5 h-3.5" /><span>帮助</span></button>
            <AnimatePresence>{helpOpen && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-full right-0 mb-2 w-80 p-3 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
      ...
      {tool && (
        <div className="px-4 py-1.5 bg-white flex items-center gap-2 border-t border-gray-100">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-purple-50 border border-purple-200 rounded-full text-xs text-purple-700">
            <Sparkles className="w-3 h-3" /><span>{tool}</span>
          ...
          <button onClick={() => setShowSettings(!showSettings)}
            ...
            <Settings2 className="w-3 h-3" /><span>更多设置</span><ChevronDown className={`w-3 h-3 transition-transform ${showSettings ? 'rotate-180' : ''}`} />
          </button>
        </div>
      )}
      ...
      <div className="px-4 py-3 border-t border-gray-200">
        <div className="flex items-center gap-2">
          <div className="flex-1">
            <input type="text" value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); send(); } }}
              placeholder={tool ? `输入您的需求...` : '输入消息...'} className="w-full h-10 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm" />
          </div>
          {hasSR && <button onClick={() => { if (!recRef.current) return; if (isRec) { recRef.current.stop(); setIsRec(false); } else { recRef.current.start(); setIsRec(true); } }}
            className={`h-10 w-10 flex items-center justify-center rounded-lg transition-colors flex-shrink-0 ${isRec ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{isRec ? <StopCircle className="w-5 h-5" /> : <Mic className="w-5 h-5" />}</button>}
...
function MsgBubble({ m, onBtn }: { m: Message; onBtn: (a: string) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className={`flex ${m.type === 'user' ? 'justify-end' : 'justify-start'}`}>
